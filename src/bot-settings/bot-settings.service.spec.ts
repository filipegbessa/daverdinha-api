...
describe('BotSettingsService', () => {
  ...
  it('get() returns the singleton row', async () => {
    prisma.botSettings.findUniqueOrThrow.mockResolvedValue({
      id: 1,
      botEnabled: false,
    });
    const result = await service.get();
    expect(result).toEqual({ id: 1, botEnabled: false });
    expect(prisma.botSettings.findUniqueOrThrow).toHaveBeenCalledWith({
      where: { id: 1 },
    });
  });
  ...
  it('update() rejects enabling the bot when there are zero active menu items', async () => {
    ...
    await expect(service.update({ botEnabled: true })).rejects.toThrow(
      BadRequestException,
    );
  ...
  it('update() allows non-enable updates without checking menu items', async () => {
    prisma.botSettings.update.mockResolvedValue({
      id: 1,
      welcomeMessage: 'Oi!',
    });
